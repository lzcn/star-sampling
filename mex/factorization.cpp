...
double diff_y_pred(DenseMat &U, DenseMat &I, DenseMat &T, uint u, uint i,
                   uint pos, uint neg) {
  uint rank = U.n_row;
  double ans = 0.0;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  for (;;) {
    ...
    for (auto itr = temp.begin(); itr != temp.end(); ++itr) {
      if (UserCount[itr->x] < 5 || ItemCount[itr->y] < 5 ||
          TagCount[itr->z] < 5) {
        // delete the tuple
  ...
  DenseMat MatItem(factorSize, itemSize);
  DenseMat MatTag(factorSize, tagSize);
  char *type;
  ...
  if (!strcmp(type, "Load")) {
    ...
    MatUser.init(MATRIX_INIT_RANDU, mean, stdev);
    MatItem.init(MATRIX_INIT_RANDU, mean, stdev);
    MatTag.init(MATRIX_INIT_RANDU, mean, stdev);
  }
    ...
    for (uint s = 0; s < Posts.size(); ++s) {
      ...
      for (uint m = 0; m < psize; ++m) {
        ...
        for (uint n = 0; n < nsize; ++n) {
          ...
          double y =
              diff_y_pred(MatUser, MatItem, MatTag, u, i, tag_pos, tag_neg);
      ...
      for (uint f = 0; f < factorSize; ++f) {
        MatUser(f, u) +=
            alpha * (MatItem(f, i) * inter_v[f] / z - lambda * MatUser(f, u));
        MatItem(f, i) +=
            alpha * (MatUser(f, u) * inter_v[f] / z - lambda * MatItem(f, i));
      }
      //----------------------------
      ...
      for (uint m = 0; m < psize; ++m) {
        ...
        for (uint f = 0; f < factorSize; ++f) {
          MatTag(f, tag_pos) +=
              alpha * (inter_q[f] * gradt - lambda * MatTag(f, tag_pos));
        }
      }
      //----------------------------
      ...
      for (uint n = 0; n < nsize; ++n) {
        ...
        for (uint f = 0; f < factorSize; ++f) {
          MatTag(f, tag_neg) +=
              alpha * (-inter_q[f] * gradt - lambda * MatTag(f, tag_neg));
        }
      }
    }
    ++saveflag;
