...
Consider the two factors, we propose a new sampling method, called central sampling. It samples the vertexes begin with partition $\overline{V}$. First, we assign a weight to each vertex in partition $\overline{V}$.
\[
    w_r = \norm{\Vacol{1}}{1}\ldots\norm{\Vacol{N}}{1}
\]
...
For the sake of comparison, we give the error bounds that similar to diamond sampling.
